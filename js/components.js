...
function MRow(p) {
  ...
  // Per-point colour ladder (green scheme): greens climb 3 -> 4 -> 5, a perfect 7 gets
  // a gold accent, 1 is a cool "wrong-result" hint, 0 is muted. bg/bd tint the whole row.
  function ptColor(v){
    return v===7 ? { fg:"#fbbf24", bg:"rgba(245,158,11,.16)",  bd:"rgba(245,158,11,.48)"  } :
           v===5 ? { fg:"#22c55e", bg:"rgba(34,197,94,.15)",   bd:"rgba(34,197,94,.42)"   } :
           v===4 ? { fg:"#4ade80", bg:"rgba(74,222,128,.11)",  bd:"rgba(74,222,128,.32)"  } :
           v===3 ? { fg:"#86efac", bg:"rgba(134,239,172,.07)", bd:"rgba(134,239,172,.24)" } :
           v===1 ? { fg:"#93c5fd", bg:"rgba(96,165,250,.07)",  bd:"rgba(96,165,250,.20)"  } :
                   { fg:thm.inv(.28), bg:thm.inv(.03), bd:thm.inv(.08) };
  }
  var pc  = pts!==null ? ptColor(pts) : null;
  var bg  = pc ? pc.bg : thm.inv(.04);
  var bd  = pc ? pc.bd : thm.inv(.09);
...
function KOMatchRow(p) {
  ...
  return html`<div style=${{
    ...
    ${matchNum?html`<span style=${{flexShrink:0,fontSize:9,fontWeight:700,color:thm.inv(.3),fontFamily:"'DM Sans',sans-serif",minWidth:24,textAlign:'left',letterSpacing:'.02em'}}>M${matchNum}</span>`:''}
    <div style=${{flex:1,display:'flex',alignItems:'center',justifyContent:'flex-end',gap:5,overflow:'hidden'}}>
      ${homeTeam
  ...
  return html`<div style=${{position:'relative',width:PW+'px',height:H+'px',flexShrink:0}}>
    ...
    ${nums.map(function(num,k){
      ...
      return html`<div key=${'m'+k} style=${{position:'absolute',top:((2*k+1)*slotH-6)+'px',left:0,right:0,height:'12px',
        ...
        <span style=${{fontSize:7,fontWeight:700,color:thm.inv(.32),background:thm.inv(.05),
