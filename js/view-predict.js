...
function PredictView(p) {
  ...
  var s8  = useState("r16");    var activeKO = s8[0], setActiveKO = s8[1];
  var s9  = useState(false);    var saving   = s9[0], setSaving   = s9[1];
  var s10 = useState(null);     var adminNotif = s10[0], setAdminNotif = s10[1];
  var s11 = useState(false);    var showScores = s11[0], setShowScores = s11[1];
  ...
  function setKOScore(key, v) {
    setPreds(function(prev){
      var ks = Object.assign({}, prev.koScores || {});
      if (v === "") delete ks[key]; else ks[key] = v;
      return Object.assign({}, prev, { koScores: ks });
    });
  }
  ...
  return html`<div className="fade" style=${{ maxWidth:780, margin:"0 auto", padding:"16px 16px 60px" }}>
    ...
    <div style=${{ display:"flex", alignItems:"center", gap:12, marginBottom:18 }}>
      ...
      <div style=${{ flex:1 }}>
        <${PBar} v=${gFilled + koFilled * 9} max=${72 + 54}/>
        <div style=${{ fontSize:11, color:"rgba(255,255,255,.3)", marginTop:3 }}>${gFilled}/72 \u00b7 ${koFilled}/6</div>
      </div>
    </div>
    ...
    ${section === "groups" && html`<${Card}>
      ...
      <div style=${{ fontSize:11, color:"rgba(255,255,255,.35)", marginBottom:8 }}>${t.groupLabel} ${activeG}: ${TBG[activeG].map(function(tm){return teamName(tm,lang);}).join(" \u00b7 ")}</div>
    ...
    ${section === "knockout" && html`<${Card} sx=${{ marginTop:14 }}>
      <div style=${{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
        <div>
          <span style=${{ fontWeight:700, fontSize:14 }}>\u26bd Score predictions</span>
          <span style=${{ fontSize:11, color:"rgba(255,255,255,.35)", marginLeft:8 }}>Optional — shown in the bracket</span>
        </div>
        <${Btn} v="secondary" onClick=${function(){ setShowScores(!showScores); }} sx=${{ padding:"5px 12px", fontSize:12 }}>
          ${showScores ? "Hide \u25b2" : "Show \u25bc"}
        </${Btn}>
      </div>
      ${showScores && (function(){
        var B=window._lastBracket;
        var sfTeams=preds.sf||[], finTeams=preds.final||[];
        var thirds=sfTeams.filter(function(x){return finTeams.indexOf(x)<0;});

        // Build all score fields from the bracket columns
        // We need to know team pairs for each round
        var r32src=preds.groups?getR32(preds.groups).teams:[];
        if(r32src.length>0 || sfTeams.length>0){
          var Bdata=typeof buildOrderedHalves!=='undefined'
            ? buildOrderedHalves(r32src,preds.r16||[],preds.qf||[],sfTeams,finTeams,preds.champion||'')
            : null;

          var rounds=[
            {key:'lSF',label:'\u2b50 Semi (left)',teams:Bdata&&Bdata.lSF||[]},
            {key:'rSF',label:'\u2b50 Semi (right)',teams:Bdata&&Bdata.rSF||[]},
            {key:'final',label:'\ud83c\udfc6 Final',teams:finTeams.slice(0,2),isFinal:true},
            {key:'s3rd',label:'\ud83e\udd49 3rd place',teams:thirds.slice(0,2),isFinal:true},
            {key:'lQF',label:'\u2b50 QF (left)',teams:Bdata&&Bdata.lQF||[],pairs:2},
            {key:'rQF',label:'\u2b50 QF (right)',teams:Bdata&&Bdata.rQF||[],pairs:2},
            {key:'lR16',label:'\ud83d\udd25 R16 (left)',teams:Bdata&&Bdata.lR16||[],pairs:4},
            {key:'rR16',label:'\ud83d\udd25 R16 (right)',teams:Bdata&&Bdata.rR16||[],pairs:4},
          ];

          return html`<div style=${{display:'flex',flexDirection:'column',gap:14}}>
            ${rounds.map(function(rd){
              if(!rd.teams||rd.teams.filter(Boolean).length<2) return null;

              if(rd.isFinal){
                var curV=(preds.koScores&&preds.koScores[rd.key])||'';
                return html`<div key=${rd.key}>
                  <div style=${{fontSize:11,color:'rgba(255,255,255,.4)',marginBottom:5,fontWeight:700}}>${rd.label}</div>
                  <div style=${{display:'flex',alignItems:'center',gap:8,background:'rgba(255,255,255,.04)',borderRadius:10,padding:'9px 12px'}}>
                    <span style=${{fontSize:12,flex:1,textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>${rd.teams[0]?fl(rd.teams[0])+' '+rd.teams[0]:''}</span>
                    <input type="text" value=${curV}
                      onInput=${function(e){setKOScore(rd.key,e.target.value);}}
                      placeholder="e.g. 2-1"
                      style=${{width:72,textAlign:'center',fontWeight:800,fontSize:14,color:'#fbbf24',letterSpacing:2,padding:'6px 4px'}}/>
                    <span style=${{fontSize:12,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>${rd.teams[1]?rd.teams[1]+' '+fl(rd.teams[1]):''}</span>
                  </div>
                </div>`;
              }

              var pairCount=rd.pairs||(rd.teams.length/2);
              return html`<div key=${rd.key}>
                <div style=${{fontSize:11,color:'rgba(255,255,255,.4)',marginBottom:5,fontWeight:700}}>${rd.label}</div>
                <div style=${{display:'flex',flexDirection:'column',gap:5}}>
                  ${Array.from({length:pairCount},function(_,pi){
                    var t1=rd.teams[pi*2]||null, t2=rd.teams[pi*2+1]||null;
                    if(!t1&&!t2)return null;
                    var scoreKey=rd.key+'_'+pi;
                    var curV=(preds.koScores&&preds.koScores[scoreKey])||'';
                    return html`<div key=${pi} style=${{display:'flex',alignItems:'center',gap:6,background:'rgba(255,255,255,.04)',borderRadius:8,padding:'6px 10px'}}>
                      <span style=${{fontSize:11,flex:1,textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:'rgba(255,255,255,.7)'}}>${t1?fl(t1)+' '+t1:'TBD'}</span>
                      <input type="text" value=${curV}
                        onInput=${function(e){setKOScore(scoreKey,e.target.value);}}
                        placeholder="?-?"
                        style=${{width:60,textAlign:'center',fontWeight:700,fontSize:13,color:'#fbbf24',letterSpacing:2,padding:'4px 2px'}}/>
                      <span style=${{fontSize:11,flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',color:'rgba(255,255,255,.7)'}}>${t2?t2+' '+fl(t2):'TBD'}</span>
                    </div>`;
                  })}
                </div>
              </div>`;
        ...
        return html`<p style=${{fontSize:12,color:'rgba(255,255,255,.3)'}}>Fill in your SF and Final picks first.</p>`;
      })()}
    </${Card}>`}
  </div>`;
}
