...
function getCascadeOpts(preds, rid, r32teams) {
  ...
  // r32 is now always auto-calculated — never user-picked
  if (rid === "r32")  return fb;
  if (rid === "r16")  return fb; // options = auto r32 qualifiers
  if (rid === "qf")   return (preds.r16  && preds.r16.length  > 0) ? preds.r16  : fb;
...
function calcScore(preds, results, sc) {
  ...
  // R32: auto-calculated from group predictions vs group results
  // Only score if results.groups has actual populated matches
  var hasGroupResults = Object.keys(results.groups||{}).some(function(k){
    var m=(results.groups||{})[k];
    return m && m.h !== '' && m.h !== undefined;
  });
  var r32hits=0, r32earned=0;
  if (hasGroupResults) {
    var predR32 = getR32(preds.groups || {}).teams;
    var resR32  = getR32(results.groups || {}).teams;
    r32hits  = predR32.filter(function(t){ return resR32.indexOf(t) >= 0; }).length;
    r32earned = r32hits * (sc.r32 || 0);
  }
  detail.r32 = { hits: r32hits, earned: r32earned }; pts += r32earned;

  // Knockout rounds (r16 onwards)
  ["r16","qf","sf","final"].forEach(function(rid) {
    var p    = preds[rid]   || [];
    var r    = results[rid] || [];
